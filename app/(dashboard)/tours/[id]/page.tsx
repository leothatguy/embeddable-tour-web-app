...
const ViewTourPage: React.FC = () => {
	...
	const tourId = Array.isArray(params.id) ? params.id[0] : (params.id as string);
	...
	if (!tour) return <p className="text-center mt-10 text-red-500">Tour not found</p>;
	...
	return (
      ...
      {/* Embed Code Section */}
      {tour.id && tour.name && (
        <EmbedCode tourId={tour.id} tourName={tour.name} />
      )}

      <h2 className="text-2xl font-semibold mt-6">Steps ({tour.steps.length})</h2>
